...
def check_winner(board):
    ...
    ai_count = 0
    moves = [[1, 0], [0, 1], [1, 1],
             [1, -1]]  # [col,row] 1. checks horizontal 2. vertical  3. diagonal +ve  4. diagonal -ve
    for m in moves:
        col_move = m[0]
        row_move = m[1]
        for c in range(cols):
...
def gui_tree(inpt, depth):
    
    
    tree_array.append((inpt, depth))
    print(depth)
    if depth == 1:
        parent_item = tree.insert("", "end", text=inpt)
    elif depth > tree_array[-2][1]:
        selected_item = tree_array[-2][0]
        parent_item = tree.parent(str(selected_item))
        tree.insert(parent_item, "end", text=inpt)
    elif depth == tree_array[-2][1]:
        selected_item = tree_array[-1][0]
        parent_item = tree.parent(selected_item)
        tree.insert(parent_item, "end", text=inpt)
    else:
        for i in range(len(tree_array) - 2, 0):
            if (depth == tree_array[i][1]):
                selected_item = tree_array[i + 1][0]
                parent_item = tree.parent(selected_item)
                tree.insert(parent_item, "end", text=inpt)
                break
    ...
    for c in range(cols):
        for r in range(rows):
            pygame.draw.rect(screen, BLUE,
                             (c * play_pos_size, r * play_pos_size + play_pos_size, play_pos_size, play_pos_size))
            pygame.draw.circle(screen, L_YELLOW, (
            int(c * play_pos_size + play_pos_size / 2), int(r * play_pos_size + play_pos_size + play_pos_size / 2)),
                               RADIUS)
    ...
    for c in range(cols):
        for r in range(rows):
            if board[r][c] == player_val:
                ...
                int(c * play_pos_size + play_pos_size / 2), height - int(r * play_pos_size + play_pos_size / 2)),
                                   RADIUS)
